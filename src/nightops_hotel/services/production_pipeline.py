...
class ProductionPipeline:
    ...
    def rows_to_records(self, rows):
        ...
        for i,row in enumerate(rows):
            missing=[f for f in self.REQUIRED_FIELDS if f not in row]
            if missing: errors.append(f"Ligne {i}: champs manquants {', '.join(missing)}"); continue
            try: persons=int(row.get('persons') or 0)
            except (TypeError, ValueError): errors.append(f"Ligne {i}: personnes invalide"); continue
